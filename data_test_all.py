...
def cal_acc(outputs, y, mask):
    ...
    return tot_acc/ARGS.train_batch
...
## load data
with open('../model_v3_concat_moreinfo/val_group_D_moreinfo_big.pkl', 'rb') as f:
        val_group = pickle.load(f)
with open('../model_v3_concat_moreinfo/test_group_D_moreinfo_big.pkl', 'rb') as f:
...
# model.load_state_dict(torch.load('addAnsw_4_0.0_24.pt')) basemodel_4_0.0_24.pt
model.load_state_dict(torch.load('basemodel_4_0.0_9.pt')) 
...
for k in dataset.keys():
    ...
    for e, item in enumerate(dataset[k]):
        ...
        if TIME_CAT_FLAG == True:
            elapsed_time = item[3].long().to(device).long()
            lag_time = item[4].long().to(device).long()
        else :
            elapsed_time = item[3].to(device).float()
            c_lag_time = item[4].to(device).float()
            p_lag_time = item[5].to(device).float()

        # exp = item[6].long().to(device).long()
        label = item[7].float().to(device).float()
        answ = item[8].to(device).long()
        diff = item[9].to(device).float()
        # popl = item[10].to(device).float()

        target_mask = (exercise != 0)

        # optimizer.zero_grad()
        output = model(exercise, part, response, elapsed_time, c_lag_time, p_lag_time, answ, diff)#, popl)
